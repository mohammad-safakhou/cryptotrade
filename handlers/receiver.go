...
type Receiver interface {
	Handler(ctx context.Context, signal string) error
}
...
func NewReceiverHandler(kucoinService *kucoin.ApiService, trader Trader) Receiver {
...
func (receiver *receiverHandler) Handler(ctx context.Context, signal string) error {
	switch signal {
	case "Short 1m":
		clientId := uuid.New()
	...
	case "Buy 1m":
		...
		fmt.Println(response.RawData)
	default:
		return errors.New("no match found for signal")
	}
	return nil
}
